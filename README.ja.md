...
| macOS | Python 3 + `python-tk`（下記参照） |

### macOS: tkinter のインストール

Homebrew 版 Python には tkinter が含まれていません。別途インストールしてください：

```bash
brew install python-tk@3.13
```

> Python のバージョンが異なる場合は読み替えてください（例: `python-tk@3.12`）。  
> python.org 版 Python は tkinter 同梱のため、追加インストール不要です。

### macOS のフォールバック動作

| Python / Tk のバージョン | 動作 |
|--------------------------|------|
| Tk 8.6+（Homebrew + python-tk / python.org 版） | 猫画像 + アニメーション表示 |
